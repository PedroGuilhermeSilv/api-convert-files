...
class ConverterService:
    ...
    def execute(self, input: InputConverter) -> OutputConverter:
        try:
            factory = self.factory_converter.create(
                InputTypeConverter(
                    typeInput=input.typeInput, typeOutput=input.typeOutput
                )
            )
            response = factory.convert(file=input.file)
            return OutputConverter(
                fileName=response.fileName, pathFile=response.filePath
            )
